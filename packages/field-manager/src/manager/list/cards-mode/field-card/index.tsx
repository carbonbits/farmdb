...
export function FieldCard({
  ...
  return (
    <button
      ...
      onClick={onSelect}
      className={[
        "flex h-full w-full flex-col gap-2.5 rounded-[13px] border bg-ivory px-[15px] pt-3.5 pb-[13px] text-left text-bark",
        isSelected
          ? "border-fern shadow-[0_0_0_1px_var(--fern)]"
          : "border-parchment hover:border-wheat hover:shadow-[0_6px_18px_rgba(20,14,9,0.08)]",
      ].join(" ")}
    >
      <span className="flex min-w-0 items-center gap-2">
        {/* The design's default field colour; it takes the crop colour once crops exist. */}
        <span aria-hidden="true" className="h-2.5 w-2.5 flex-none rounded-[3px] bg-meadow" />
        <span className="truncate text-[15px] font-bold tracking-[-0.2px]">{field.name}</span>
      </span>
      <span className="-mt-0.5 line-clamp-2 text-[12.5px] leading-[1.4] text-umber">
        {field.description || "No crop recorded"}
      </span>
      <span className="grid grid-cols-2 gap-x-2.5 gap-y-[9px] border-t border-parchment pt-[11px]">
        <CardFact
          label="Area"
...
function CardFact({
  ...
  return (
    <span className="block min-w-0">
      <span className="block text-[10px] font-bold tracking-[0.7px] text-taupe uppercase">
        {label}
      </span>
      <span
        className={[
