...
export function FieldPanel({ field, onClose }: { field: Field; onClose: () => void }) {
  ...
  return (
    <section
      ...
      className="h-full w-full overflow-y-auto rounded-[10px] border border-white/65 bg-white p-5 text-bark shadow-[0_20px_54px_rgba(20,14,9,0.28)]"
    >
      <header className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[10px] font-bold tracking-[1.4px] text-taupe uppercase">Field</p>
        ...
        <button
          ...
          className="flex h-10 w-10 flex-none items-center justify-center rounded-[11px] border border-parchment bg-white text-soil hover:bg-cream"
        >
          <CloseIcon />
        </button>
      </header>
      <p className="mt-1 text-[13px] text-umber">{field.description || "No crop recorded"}</p>
      ...
      <div className="border-t border-parchment pt-3.5">
        <p className="mt-1.5 mb-1 text-[11px] font-bold tracking-[1.2px] text-taupe uppercase">
          Records derived from this field
        </p>
        <div className="mt-1.5 flex flex-wrap gap-1 rounded-[12px] bg-cream p-1">
          {RECORD_TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={[
                  ...
                  isActive ? "bg-white text-bark shadow-[0_1px_3px_rgba(0,0,0,0.1)]" : "text-umber",
                ].join(" ")}
              >
                {tab.label}
              </button>
        ...
        <p className="mt-1 border-t border-parchment py-[13px] text-[13px] text-taupe">
...
function PanelFact({
  ...
  return (
    <div className="min-w-0">
      <dt className="text-[10.5px] font-bold tracking-[0.8px] text-taupe uppercase">{label}</dt>
      <dd
        className={[
          "mt-[3px] truncate text-[14.5px]",
          isMissing ? "font-normal text-taupe" : "font-semibold",
        ].join(" ")}
      >
        {value}
      </dd>
